...
if __name__ == "__main__":
    ...
    l_pos_q = task_share.Queue("l", 1000, thread_protect=False, overwrite=True, name="left position queue")
    r_pos_q = task_share.Queue("l", 1000, thread_protect=False, overwrite=True, name="right position queue")
    l_vel_q = task_share.Queue("l", 1000, thread_protect=False, overwrite=True, name="left velocity queue")
    r_vel_q = task_share.Queue("l", 1000, thread_protect=False, overwrite=True, name="right velocity queue")

    ## Create objects of each task for the Task objects
    user_input_obj = UserInput()
    LMC_obj = MotorController(l_motor, l_encoder)
    RMC_obj = MotorController(r_motor, r_encoder)
