...
def plot_pairs(file_path, svg_folder, pairs, delineator):
    ...
    for vs, symb, units, square in pairs:
        ...
        plt.xlabel(fr"{x_label}, ${x_symb}$, [{x_units}]")
        plt.ylabel(fr"{y_label}, ${y_symb}$, [{y_units}]")
...
def main():
    ...
    file_name = Path(__file__).resolve().parent / "First Circle Data.csv"
    pairs = [((None, "obsd_cpos"), ("i", "s"), ("-", "mm"), False), ((None, "IMU heading"), ("i", "\psi"), ("-", "rad"), False), (("obsd_X_pos", "obsd_Y_pos"), ( "X", "Y"), ("mm", "mm"), True)]
    plot_pairs(file_name, svg_folder, pairs, "First Circle")

    file_name = Path(__file__).resolve().parent / "Second Circle Data.csv"
    plot_pairs(file_name, svg_folder, pairs, "Second Circle")
