...
class LineSensor(Sensor):
    ...
    # Sensor Gains
    sensor_Kp = 0.10  # Proportional gain
    sensor_Ki = 1000  # Integral gain
    ...
    def get_data(self) -> float:
        ...
        print(self.buffer, sum_pos_val / sum_val)
        ...
        if self.use_cal and "IR_cal.txt" in listdir():
            ...
            with open("IR_cal.txt", "r") as file:
                ...
                if len(white_data) != len(black_data) or len(white_data) != len(self.list_IR_Sensors):
                    raise ValueError("Calibration data length does not match number of IR sensors.")
